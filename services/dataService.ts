...
export const dataService = {
  ...
  getMonthlyReport: (year: number, month: number) => {
    ...
    // Filtrar logs del mes seleccionado o todo el año si month = -1
    ...
    // Mapear schedules por id para referencia rápida
    const scheduleMap = new Map(schedules.map(s => [s.id, s]));

    // Agregar nombre del schedule a cada log y ordenar por fecha descendente
    const logsWithScheduleName = monthLogs.map(log => ({
      ...log,
      scheduleName: scheduleMap.get(log.scheduleId)?.name || 'Respaldo desconocido'
    })).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());

    return logsWithScheduleName;
  }
};
